...
class Step2Edit(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        for col, (key, label) in enumerate(
            ...
            ctk.CTkLabel(
                f, text=label, font=ctk.CTkFont(size=11), text_color="#888888"
            ).pack()
            sl = ctk.CTkSlider(
                f,
                from_=-12,
        ...
        ctk.CTkLabel(lim_inner, text="Target LUFS:", font=ctk.CTkFont(size=12)).pack(
            side="left"
        )

        self._lufs_slider = ctk.CTkSlider(
            lim_inner, from_=-24, to=-6, width=200, command=self._on_lufs_change
        )
        self._lufs_slider.set(self.session.get("lufs_target", -14.0))
        self._lufs_slider.pack(side="left", padx=10)

        self._lufs_label = ctk.CTkLabel(
            lim_inner, text="-14.0 LUFS", font=ctk.CTkFont(size=12)
        )
        ...
        self._lufs_current = ctk.CTkLabel(
            lim_inner, text="", text_color="#888888", font=ctk.CTkFont(size=11)
        )
        ...
        ctk.CTkButton(nav, text="Siguiente →", command=self.on_next, width=140).pack(
            side="right"
        )
    ...
    def on_enter(self):
        ...
        self._player.load(
            audio, self.session["sample_rate"], self.session.get("processed_audio")
        )
    ...
    def _reprocess(self):
        ...
        def _work():
            try:
                ...
                limited = apply_limiter(
                    eq_out, self.session["sample_rate"], lufs_target
                )
                ...
                self.after(
                    0,
                    lambda lufs_val=lufs: self._lufs_current.configure(
                        text=f"actual: {lufs_val:.1f} LUFS", text_color="#888888"
                    ),
                )
                ...
                if gen == self._process_gen:
                    self.after(
                        0,
                        lambda err=str(e): self._lufs_current.configure(
    ...
    def _update_lufs_display(self):
        ...
        self._lufs_current.configure(
            text=f"actual: {lufs:.1f} LUFS", text_color="#888888"
        )
